...
fn test_close_validates_status() {
    ...
    // verify close cannot be run before close_ledger
...
#[test]
fn test_close_blnd_bootstrap_single_sided_blnd() {
    let e = Env::default();
    e.budget().reset_unlimited();
    e.mock_all_auths();
    e.set_default_info();
...
#[test]
fn test_close_usdc_bootstrap_single_sided_usdc() {
    let e = Env::default();
    e.budget().reset_unlimited();
    e.mock_all_auths();
    e.set_default_info();
    ...
    // blnd -> 50k
    // uscd -> 2k
    // approx 750 extra usdc
    let bootstrap_amount = 2000 * SCALAR_7;
    ...
    // join bootstrap
    let join_amount = 50000 * SCALAR_7;
    blnd_client.mint(&samwise, &join_amount);
    bootstrap_client.join(&samwise, &id, &join_amount);
